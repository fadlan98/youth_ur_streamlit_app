...
if view == "Home":
    ...
    st.write(
        "Based on the ETS forecast, youth unemployment is expected to remain fairly stable over the next year, "
        "with no large jumps relative to recent months."
    )

elif view == "ARIMA Forecast":
    st.subheader("ARIMA — Hold-out and Forecast (Notebook-style)")
    H  = st.sidebar.slider("Hold-out months (H)", 6, 24, 12, 1)
    fh = st.sidebar.slider("Forecast horizon (months)", 6, 24, 12, 1)
